...
// [TODO] https://learnopengl.com/Advanced-OpenGL/Advanced-GLSL 
#define NB_LIGHTS 2
layout (location = 2) uniform Light lights[NB_LIGHTS];
uniform uint usedLightCount;

vec3 LightCalculation(Light l, vec3 n, Vertex v) // light, normal, vertex
{
	vec3 uv = vec3(texture(tex, v.uv));
	vec3 ambiant = uv * l.ambiant;

	vec3 direction = normalize(l.position - v.position);
	float difference = max(dot(n, direction), 0.0);
